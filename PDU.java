    ...
    public static PDU create(ByteBuffer buffer, ByteChannel src) {
        ...
        switch(t) {
            ...
            case NET_GET_NODE_RESPONSE:
                p = new NET_GET_NODE_RESPONSE(buffer, src);
                break;
            case NET_JOIN:
                p = new NET_JOIN(buffer, src);
                break;
            case NET_JOIN_RESPONSE:
                p = new NET_JOIN_RESPONSE(buffer, src);
                break;
            case VAL_INSERT:
                p = new VAL_INSERT(buffer, src);
                break;
            case VAL_LOOKUP:
                p = new VAL_LOOKUP(buffer, src);
                break;
            case VAL_LOOKUP_RESPONSE:
                p = new VAL_LOOKUP_RESPONSE(buffer, src);
                break;
        }
